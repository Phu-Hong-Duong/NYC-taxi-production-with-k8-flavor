...
| **R-2** | **An alert on an empty or stale online store** — the residual M8-S4 legs 1, 2 and 3 each restated and none closed, now with the community's per-request half attached (row 3; credit **F**) and its TTL half named (row 5). | routed to the M8 boundary; it belongs to the story that puts a reader in front of the store | It is a new monitored signal with a threshold, and a threshold argued in the same session that first measures the thing it watches is the pattern M8 law 4 exists to forbid. It needs its own headroom leg. **DONE 2026-08-23 (M9-S2) — and the "it needs its own headroom leg" clause is what the story was: `automation/runs/m9-store-watch/headroom.json` was recorded and `docs/slo_serving.md` §9 written from it BEFORE any rule existed, checkable from git. The headroom's central finding then removed the threshold this row assumed: the store's 57,688 keys are exactly the sum of distinct entity keys across the published sources, so A-12b compares the store against its own sources and carries no number at all. Landed as A-12 (SLO-S1) + A-13 (SLO-S2). The per-request half of row 3 is NOT closed — F-062, OPEN.** |
...
**Status 2026-08-23 (M9-S2): R-2 is DONE and it is the only one of the three that
has moved.** R-1 and R-3 are unchanged and still unscheduled — recorded here
because a survey page whose routing table is never revisited is a page that
credits itself for work nobody did. Worth one line for whoever reads this next:
R-2 was routed on the argument that it *"needs its own headroom leg"*, and the
headroom leg is the part that paid — it is what turned a signal everyone expected
to carry a key-count threshold into two rules that carry **no number on either
side of the comparison**. The routing note was right about the cost and wrong
about the shape, which is the usual yield of doing the leg instead of estimating
it.
