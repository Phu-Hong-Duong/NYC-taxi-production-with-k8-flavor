...
| Host test suite | **1,408 tests**, no skips | `uv run pytest tests/unit -q` |
...
| **Followability cleanup (CU)** — PO directive 2026-08-29-2, five slices over the verified audit: scope rule + dead code · one test conftest · verify-harness + red-team-restore libs · python plumbing · isvc deploy lib + the report; the cleanup is a subtraction of copies, not of guarantees | **closed 2026-08-31** | tag `cleanup-closed` · `docs/cleanup_report.md` (the directive's write-up) · all ten gates re-run GREEN at the close by the approver · the full red-team battery RED-then-GREEN on every drill · `docs/milestones/PROGRAM_CLOSE.md` §8 |
