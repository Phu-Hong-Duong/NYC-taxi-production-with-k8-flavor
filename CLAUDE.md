...
| The split as Prometheus draws it (M6-S4) | `uv run python scripts/canary_split_paste.py [--minutes N]` (a READER — one range query) | VERIFIED 2026-08-19 (M6-S4): prints champion/canary/share per minute; the green run reads `5.9% · 11.1% · 100.0% · 100.0% · 0.0%` at 240 req/min, with the failed attempt's flat `0.0%` visible six minutes above it. It exists so the board and the record are not two different claims. **RETIRED at CU-S1 (2026-08-31): the script is deleted; the split it printed is banked in `automation/runs/m6-canary/` and re-derivable from Prometheus by hand — what is gone is the affordance, not the evidence** |
